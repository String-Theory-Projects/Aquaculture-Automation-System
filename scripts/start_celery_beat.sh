...
# Set default port for beat health server (8081 to avoid conflict with worker)
...
$PYTHON_CMD manage.py celery_beat_health > /tmp/celery_beat_health.log 2>&1 &
HEALTH_PID=$!
...
if ! kill -0 $HEALTH_PID 2>/dev/null; then
    ...
    echo "Check /tmp/celery_beat_health.log for details"
    cat /tmp/celery_beat_health.log 2>/dev/null || true
    exit 1
fi
...
exec $PYTHON_CMD -m celery -A FutureFish beat -l warning
