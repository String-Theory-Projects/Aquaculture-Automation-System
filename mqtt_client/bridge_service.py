...
class MQTTBridgeService:
    ...
    def _create_simplified_message(self, command_id: str, command_type: str, pond_position: int, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if command_type == 'WATER_FLUSH':
            return {
                ...
                'type': 'FLUSH',
                'pos': pond_position,
                'd': parameters.get('drain_level', 0.0),  # drain distance
                'f': parameters.get('fill_level', 80.0)   # fill distance
            }
    ...
    def send_water_command(self, pond_pair: PondPair, action: str, level: float = None, 
        ...
        if level is not None and pond:
            level = pond.percentage_to_sensor_distance(level)
        
        # Handle WATER_FLUSH special case with drain and fill levels
        if action == 'WATER_FLUSH':
            drain_level = kwargs.get('drain_level')
            fill_level = kwargs.get('fill_level')
            if drain_level is not None and pond:
                drain_level = pond.percentage_to_sensor_distance(drain_level)
            if fill_level is not None and pond:
                fill_level = pond.percentage_to_sensor_distance(fill_level)
            kwargs['drain_level'] = drain_level
            kwargs['fill_level'] = fill_level
