...
from dashboard.serializers.profile_serializers import PondSerializer, PondRegistrationSerializer, WiFiConfigSerializer
...
class RegisterPondView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if Pond.objects.filter(owner=request.user, name=pond_name, is_active=True).exists():
                return Response(
    ...
    def put(self, request, pk):
        ...
        if serializer.is_valid():
            ...
            if wifi_data:
                try:
                    wifi_config = pond.wifi_config
                    wifi_serializer = WiFiConfigSerializer(wifi_config, data=wifi_data, partial=True)
                except WiFiConfig.DoesNotExist:
                    # Create new WiFi config if it doesn't exist
                ...
                if wifi_serializer.is_valid():
                    ...
                    return Response(wifi_serializer.errors, status=status.HTTP_400_BAD_REQUEST)
            
            return Response({
                'message': 'Pond updated successfully',
                'pond': serializer.data
            })
