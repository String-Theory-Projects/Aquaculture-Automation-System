...
from dashboard.models import Pond, PondControl, SensorData
from dashboard.serializers.analytics_serializers import (
    CurrentSensorDataSerializer, HistoricalDataSerializer,
    PondControlSerializer, PondDropdownSerializer)
...
class DashboardDataView(generics.RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not pond_id:
            return Response(
                {"error": "Pond ID is required"}, status=status.HTTP_400_BAD_REQUEST
            )

        try:
            # Get latest sensor and pond reading
            latest_sensor_reading = SensorData.objects.filter(pond_id=pond_id).latest(
                "timestamp"
            )

            latest_pond_reading = PondControl.objects.filter(pond_id=pond_id).last()
            ...
            return Response(
                {"error": "No sensor data available"}, status=status.HTTP_404_NOT_FOUND
            )
...
class HistoricalDataView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        sensor_data = SensorData.objects.filter(
            pond=pond, timestamp__gte=start_time, timestamp__lte=now
        )
        ...
        while current_interval < now:
            ...
            interval_data = sensor_data.filter(
                timestamp__gte=current_interval, timestamp__lt=next_interval
            ).aggregate(
        ...
        # Return the response
        return Response({"historical_data": serializer.data, "time_range": time_range})
...
class UserPondsDropdownView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return Pond.objects.filter(owner=self.request.user, is_active=True)
