...
class CustomTokenObtainPairViewTest(TestCase):
    ...
    def test_login_with_email(self):
        ...
        payload = {"username": "test@example.com", "password": "TestPassword123!"}
        response = self.client.post(self.login_url, payload, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIn("access", response.data)
        self.assertIn("refresh", response.data)
    ...
    def test_login_with_invalid_credentials(self):
        ...
        payload = {"username": "testuser", "password": "WrongPassword123!"}
        response = self.client.post(self.login_url, payload, format="json")
    ...
    def test_login_includes_pond_information(self):
        ...
        Pond.objects.create(
            name="Test Pond", owner=self.user, device_id="testdevice123"
        )
...
class RegisterPondViewTest(TestCase):
    ...
    def test_register_pond_success(self):
        ...
        self.assertTrue(
            WiFiConfig.objects.filter(pond__device_id="testdevice123").exists()
        )

    def test_register_pond_without_wifi(self):
        """Test that a pond can be registered without WiFi credentials"""
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.access_token}")
        payload = {"name": "Test Pond", "device_id": "testdevice456"}
        ...
        self.assertFalse(
            WiFiConfig.objects.filter(pond__device_id="testdevice456").exists()
        )
    ...
    def test_register_duplicate_device_id(self):
        ...
        # Create a pond first
        Pond.objects.create(
            name="Existing Pond", owner=self.user, device_id="testdevice123"
        )
...
class ChangePasswordViewTest(TestCase):
    ...
    def test_change_password_success(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.access_token}")
        payload = {"old_password": "OldPassword123!", "new_password": "NewPassword456!"}
        ...
        self.assertTrue(
            self.client.login(username="testuser", password="NewPassword456!")
        )
    ...
    def test_change_password_weak_new(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.access_token}")
        payload = {"old_password": "OldPassword123!", "new_password": "weak"}
        response = self.client.post(self.change_password_url, payload, format="json")
