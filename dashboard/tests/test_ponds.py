...
class PondListViewTest(TestCase):
    ...
    def setUp(self):
        ...
        Pond.objects.create(
            name="First Pond", owner=self.user, device_id="device001", is_active=True
        )

        Pond.objects.create(
            name="Second Pond", owner=self.user, device_id="device002", is_active=True
        )
    ...
    def test_get_pond_list(self):
        ...
        self.assertEqual(
            len(response.data), 3
        )  # Only user-owned ponds, not system-owned
...
class PondDetailViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.pond = Pond.objects.create(
            name="Test Pond", owner=self.user, device_id="testdevice123", is_active=True
        )
    ...
    def test_update_pond(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.access_token}")
        payload = {"name": "Updated Pond Name"}
    ...
    def test_update_is_active_ignored(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.access_token}")
        payload = {"name": "Updated Name", "is_active": False}  # This should be ignored
        response = self.client.put(self.pond_detail_url, payload, format="json")
...
class PondDetailViewUniqueNameTest(TestCase):
    ...
    def test_update_to_duplicate_name(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.access_token}")
        payload = {"name": "Second Pond"}  # Same as pond2's name
    ...
    def test_update_to_unique_name(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.access_token}")
        payload = {"name": "Updated Pond Name"}  # Unique name
        response = self.client.put(self.pond1_url, payload, format="json")
    ...
    def test_update_to_same_name(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.access_token}")
        payload = {"name": "First Pond"}  # Same as current name
        response = self.client.put(self.pond1_url, payload, format="json")
    ...
    def test_update_with_special_characters(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.access_token}")
        payload = {"name": "Koi & Goldfish Pond (2023) - Backyard"}
        response = self.client.put(self.pond1_url, payload, format="json")
...
class RegisterPondViewTest(TestCase):
    ...
    def test_register_pond_success(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.access_token}")
        response = self.client.post(
            self.register_pond_url, self.valid_payload, format="json"
        )

        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(Pond.objects.filter(device_id="testdevice123").exists())
        self.assertTrue(
            PondControl.objects.filter(pond__device_id="testdevice123").exists()
        )
    ...
    def test_register_without_wifi(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.access_token}")
        payload = {"name": "No WiFi Pond", "device_id": "nowifi123"}
    ...
    def test_other_user_cannot_reactivate(self):
        ...
        user_pond = Pond.objects.create(
            name="User Pond", owner=self.user, device_id="userdevice123", is_active=True
        )
