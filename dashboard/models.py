...
class WiFiConfig(models.Model):
    ...
    pond = models.OneToOneField(
        Pond, on_delete=models.CASCADE, related_name="wifi_config"
    )
    ...
    is_config_synced = models.BooleanField(
        default=False, help_text="Whether config has been synced to device"
    )
...
class SensorData(models.Model):
    ...
    pond = models.ForeignKey(
        Pond, on_delete=models.CASCADE, related_name="sensor_readings"
    )
...
class AutomationSchedule(models.Model):
    ...
    pond = models.ForeignKey(
        Pond, on_delete=models.CASCADE, related_name="automation_schedules"
    )
    ...
    feed_amount = models.FloatField(
        null=True, blank=True, help_text="Feed amount in grams"
    )
...
class DeviceLog(models.Model):
    ...
    command_id = models.UUIDField(
        null=True, blank=True, help_text="Reference to associated MQTT command"
    )
    retry_count = models.IntegerField(
        default=0, help_text="Number of retries for automation commands"
    )
...
class MQTTMessage(models.Model):
    ...
    pond = models.ForeignKey(
        "Pond", on_delete=models.CASCADE, related_name="mqtt_messages"
    )
    topic = models.CharField(
        max_length=255, help_text="MQTT topic used for the message"
    )
    payload = models.JSONField(help_text="Message payload in JSON format")
    message_type = models.CharField(
        max_length=7, choices=MESSAGE_TYPES, help_text="Type of MQTT message"
    )
    status = models.CharField(max_length=8, choices=STATUS_CHOICES, default="SENT")
    timestamp = models.DateTimeField(
        default=timezone.now, help_text="When the message was sent/received"
    )
    retries = models.IntegerField(
        default=0, help_text="Number of retry attempts for automation commands"
    )
