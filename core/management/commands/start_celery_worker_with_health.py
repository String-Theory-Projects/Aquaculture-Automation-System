"""
Django management command to start Celery worker with health check server.

This command runs both the Celery worker and a health check HTTP server.
The health server runs in a background thread, and Celery worker runs in the main thread.
"""
...
from core.management.commands.celery_worker_health import Command as HealthCommand

logger = logging.getLogger(__name__)


class Command(BaseCommand):
    help = 'Start Celery worker with health check server'
    ...
    def handle(self, *args, **options):
        """Start Celery worker and health server"""
        self.stdout.write(
            self.style.SUCCESS('Starting Celery worker with health server...')
        )
        ...
        # Start Celery worker in foreground (blocking)
    ...
    def _start_celery_worker(self):
        """Start Celery worker"""
        try:
            # Get Celery worker command from environment or use default
            worker_command = os.environ.get(
                'CELERY_WORKER_COMMAND',
                'celery -A FutureFish worker -l warning -Q automation,mqtt,analytics --concurrency=4'
            )
            
            # Split command into parts
            cmd_parts = worker_command.split()
            
            self.stdout.write(
                self.style.SUCCESS(f'Starting Celery worker: {" ".join(cmd_parts)}')
            )
    ...
    def _cleanup(self):
        ...
        if self.celery_process:
            try:
                ...
                logger.warning(f'Error terminating Celery worker: {e}')
                try:
